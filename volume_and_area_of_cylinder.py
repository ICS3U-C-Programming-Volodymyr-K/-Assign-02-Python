...
def main():
    # Welcomes the user.
    print(
        "Hello, the user of this calculator. This calculator may be useful for you in order to find the surface area and volume of cylinder. "
    )
    # Asks user about their next action.
    welcome_message = input(
        "If you want to continue calculating, type yes and if not then type no:"
    )
    ...
    if welcome_message.lower() == "no":
        ...
        question_metrics = input(
            "To what metrics do you want to convert your values? (m/mm/km), if you do not wish to have your values converted, say no: "
        )
        ...
        if question_metrics == "m":
            ...
            print(
                "The surface area of cylinder is {: .5f} meters squared".format(
                    metrical_surface_area
                )
            )
            print(
                "The volume of cylinder is {: .5f} meters cubed".format(metrical_volume)
            )
        # If user chooses to convert to mm, the code multiplies the volume and surface area by 10
        elif question_metrics == "mm":
            millimetrical_surface_area = surface_area * 10
            millimetrical_volume = volume * 10
            # Displays the converted values in millimeters
            print(
                "The surface area of cylinder is {: .5f} millimeters squared".format(
                    millimetrical_surface_area
                )
            )
            print(
                "The volume of cylinder is  {: .5f} millimeters cubed".format(
                    millimetrical_volume
                )
            )
        # If the user decides to convert the values into kilometers, the program will divide it by a hundred thousand
            ...
            print(
                "The surface area of cylinder is {} kilometers squared".format(
                    kilometrical_surface_area
                )
            )
            print(
                "The volume of cylinder is {} kilometers cubed".format(
                    kilometrical_volume
                )
            )
